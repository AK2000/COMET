...
for(Value pos : lhs.getPos()){
    ...
    candidates.insert(std::make_pair(index_to_tensor.getIndex(), index_to_tensor.getLevel()));
}
...
for(Value rhs : compute_op.getRhs()){
    ...
    for(; dim < positions.size(); dim += 1) {
        auto index_to_tensor = positions[dim].getDefiningOp<IndexTreeIndexToLevelOp>();
        Value index = index_to_tensor.getIndex();
